...
# 2013-1-24
					...
					my $failString = "Success";
					 ...
					 if($valid)
					 {
						...
						#Logging and emailing
							...
							local $@;
							eval{$sierraSpecial = new sierraSpecial($dbHandler,$log,$selectQuery,$type,$conf{"school"},$pathtothis,$configFile,$maxdbconnections);};
							if($@)
							{
								$valid=0;
							...
							my $gatherTime = $sierraSpecial->calcTimeDiff($gatherTime);
							...
							if($valid)
							{
								my @all = @{$sierraSpecial->getAllMARC()};
								my @marc = @{@all[0]}; 
								my @tobig = @{$sierraSpecial->getTooBigList()};
 ...
 sub thread
 {
	...
	else
	{
		...
		eval{$sierraSpecial = new sierraSpecial($dbHandler,$log,$selectQuery,$type,$conf{"school"},$pathtothis,$configFile);};
		if($@)
		{
		...
		my @diskDump = @{$sierraSpecial->getDiskDump()};
		my $disk =@diskDump[0];
		my @marc =();
		my $check = new Loghandler($disk);
		my $recordCount = $sierraSpecial->getRecordCount();
		if(!$check->fileExists())
		{
			$disk='';
			$recordCount=0;
		...
		my $queryTime = $sierraSpecial->getSpeed();
		my $secondsElapsed = $sierraSpecial->calcTimeDiff($previousTime);
