...
my $ageToCompress = @ARGV[2];
my $ageToDelete = @ARGV[3];
my $toemail = @ARGV[4];
...
if(1)
{
	for my $i(0..$#files)
	{
		...
		if($diff>$ageToCompress && !(-d $thisFile))
		{
			...
			#print "Flagged for compression: $thisFile age $diff\n";
				...
				my $dest = $mobutil->chooseNewFileName($folder,$thisFileName,"gz");
				open($n,"<",$thisFile);
		...
		if(!$compressed)
		{
			if($diff>$ageToDelete)
			{
				if(-d @files[$i])
				{
					...
					opendir(DIR,@files[$i]) or die "Cannot open @files[$i]\n";
	...
	if($somethingChanged)
	{
		...
		my $htmlOut = '<div class="job"><div class="jobheader">'.@s[0].'   -  '.@s[1].'</div><div class="section">Files Deleted</div>';
		...
		my @tolist = ($toemail);
		my %conf = ();
		my $email = new email("dropbox\@mobiusconsortium.org",\@tolist,0,0,\%conf);
...
sub getHTMLHead
{
	...
	my $htmlStart = '<html><head><title>Deleted files from MOBIUS share</title>'.$style.'</head><body><div class="allhead">Old Files Removal Activity Log</div>'."\n";
	return $htmlStart;
}
