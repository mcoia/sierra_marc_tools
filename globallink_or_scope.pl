...
# This 
 ...
 if($conf)
 {
	...
	# If a prompt is not found, the rest of the array is skipped and the next array begins
				...
				if($cluster eq 'explore' or $cluster eq 'swbts' or $cluster eq 'swan' or $cluster eq 'archway' or $cluster eq 'arthur' or $cluster eq 'bridges' or $cluster eq 'avalon' or $cluster eq 'kc-towers')
				{
					@firstPrompts = (
					10,"Login",$conf{"secondlogin"}."\r",1,
					10,"Password",$conf{"secondpassword"}."\r",1,					
